...
true_state_hist = torch.zeros(replay_size,10)
...
for t=1,max_steps do
    ...
    local opfunc = function(x)
        ...
        qPrime_hist = network:forward(state_prime_hist[state_bind]:reshape(batch_size,10)):clone()
        q = network:forward(state_hist[state_bind]:reshape(batch_size,10)):clone()
        target = q:clone()
        ...
        network:backward(state_hist[state_bind]:reshape(batch_size,10),grad)
    ...
    if t%interval == 0 then
        print(t,total_reward/interval,total_loss,gradients:norm(),state_counts)
        state_counts:zero()
        total_reward = 0
        total_loss = 0
    end
